...
![**低版本Hadoop副本选择**](https://github.com/ChocolateLi/MyKnowledgeRepository/blob/main/picture/%E4%BD%8E%E7%89%88%E6%9C%ACHadoop%E6%9C%BA%E6%9E%B6%E9%80%89%E6%8B%A9.png?raw=true)
...
![Hadoop 2.7.2副本节点选择](https://github.com/ChocolateLi/MyKnowledgeRepository/blob/main/picture/hadoop2.7.2%E7%89%88%E6%9C%AC%E7%9A%84%E5%89%AF%E6%9C%AC%E9%80%89%E6%8B%A9.png?raw=true)



### 5、MapReduce的执行过程

![MapReduce的执行过程](/Users/chenli75/Desktop/MyFile/github/MyKnowledgeRepository/internship_picture/mapreduce执行过程.png)
...
3、一般来讲倾斜的key都很少，可以将他们抽样出来，对应的行单独存入临时表中，然后打上随机前缀，最后再进行聚合。
...
RDD是一个懒执行的、不可变的、支持Lambda表达式的并行数据集合。它的优点是简单易用，但它的缺点是序列化和反序列成本比较高
...
![Flink的组件栈](/Users/chenli75/Desktop/MyFile/github/MyKnowledgeRepository/picture/Flink组件栈.png)

Flink是一个分层架构系统，每一层所包含的组件都提供特定的抽象，用于服务上层组件。

flink组件栈包括deploy层、runtime层、API层、libraries层。

deploy层：该层涉及到Flink的部署模式，支持 local、standalone、cluster、cloud等多种部署模式。
...
![Flink的基础编程模型](/Users/chenli75/Desktop/MyFile/github/MyKnowledgeRepository/picture/Flink基础编程模型.png)
...
flink集群有jobsmanager、taskmanager、client三个角色。
...
flink实现多种重启策略。固定延迟重启策略、故障率重启策略、无重启策略、后备重启策略
