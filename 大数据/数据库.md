...
-- HIVE_USER密码过期
ALTER USER hive_user IDENTIFIED BY hive_0753;

SELECT username, account_status, expiry_date
FROM dba_users
WHERE username = 'HIVE_USER';
...
```sql
# 创建用户
CREATE USER 'javauser'@'%' IDENTIFIED BY 'javauser_0753';

# 创建数据库
create database classify;

# 授予权限
GRANT ALL PRIVILEGES ON classify.* TO 'javauser'@'%';
