...
Container：资源的抽象，里面封装了CPU、内存等资源。（一个container运行一个task）
...
![](D:\Github\MyKnowledgeRepository\img\bigdata\spark\yarn-cluster模式.jpg)

客户端向RM提交申请。作业提交上来后，RM首先在所有的NodeManager里面分配一个container出来，用来运行ApplicationMaster。但是Spark的管理进程是driver，Yarn的管理进程是ApplicaitonMaster，那他们怎么协调呢？只需要将driver外面套一个ApplicationMaster壳子就行。运行起来之后，负责解析作业的是driver。driver把spark 作业解析成了8个task，意思是需要8个task的资源。但是driver没办法申请hadoop的资源呀，那driver通知ApplicationMaster去申请资源，于是ApplicaitonMaster向RM申请资源，RM就把资源container分配给了ApplicaitonMaster，然后ApplicaitonMaster就会通知driver资源给你申请到了。driver就会把解析好的task分发给这些Executor执行，准确来说是container执行，一个container执行一个task。（在yarn上叫contianer，在spark上叫executor）。执行过程中，task会实时向driver汇报，driver发现所有task运行完成之后，再通知ApplicationMaster可以释放资源了，于是RM就把所有资源释放掉了。

#### yarn-client和yarn-cluster模式的不同之处

YarnClient模式下，driver运行在本地机器。适用于交互和调试

YarnCluster模式下，driver运行在集群某个NodeManager的节点上面。适用于生产环境
...
2. 时间机制：SparkStreaming只支持有限的时间机制。Flink支持三种：事件时间、注入时间、处理时间。同时也支持watermark机制来处理滞后的数据
...
遍历文件a，对每个url求取hash(url)%1000，然后根据所取得的值将url分别存储到1000个小文件（记为a0,a1,...,a999）中。保证每个小文件能够加载到内存中。

遍历文件b，采取同样的方式。

这样处理后，所有可能相同的url都在对应的小文件中，因为相同的url 哈希之后一定在同一个小文件中（a0 vs b0,a1 vs b1,...）。

求每对小文件时，可以把其中一个小文件的url存储到hash set中。然后遍历另一个小文件的每个url，看其是否在刚才建的hash set中，如果是，那么就是共同的url，存储到文件中。
