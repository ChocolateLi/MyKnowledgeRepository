...
5. B树：优点：每个节点可以有多个子节点，可以控制树的高度，比二叉平衡树减少磁盘I/O次数。缺点：数据库索引是存储在磁盘上的，如果数据很多会导致很大，当加载索引的时候，只能一页页加载磁盘页，加大磁盘I/O的次数。（它的非叶子节点也存储了数据）
...
3. 让选择性强的索引放在前面
4. 前缀索引。对于Text、Varchar类型的列使用前缀索引效率高
...
MVCC的思想就是保存历史版本，通过对数据行多个版本的管理来实现数据库的并发控制。

它的解决以下问题：

1. 通过MVCC可以让读写不阻塞，提高事务的并发能力。
