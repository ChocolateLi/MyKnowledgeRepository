...
在模型的训练过程中，我们可以使用二元交叉熵损失函数来衡量预测结果和真实结果之间的差距，并使用反向传播算法来更新模型的参数。
...
在上述代码中，`targets`表示真实标签，是一个形状为`(3,)`的一维张量，其中包含了三个样本的真实标签。由于PyTorch的`nn.CrossEntropyLoss`函数会自动将真实标签转化为概率分布的形式，因此这里可以直接将真实标签以整数形式传递给函数即可。在计算损失函数时，函数会自动将真实标签转化为概率分布的形式进行计算，而不需要手动进行转化。
...
在这个例子中，模型的输出是一个形状为`(3,2)`的张量，代表3个样本的概率。标签是一个形状为`(3,)`的张量，表示3个样本的标签。我们创建了一个`torch.nn.CrossEntropyLoss`函数的实例，并将模型输出和标签作为输入传递给该函数，最终得到损失值。
...
[Chung J, Gulcehre C, Cho K, et al. Empirical evaluation of gated recurrent neural networks on sequence modeling[J\]. arXiv: Neural and Evolutionary Computing, 2014.](https://link.zhihu.com/?target=https%3A//arxiv.org/pdf/1412.3555.pdf)
...
![](D:\github\MyKnowledgeRepository\img\machine_learning_img\deep_learning\实验结果\验证集准确率在0.75的实验结果.png)

验证集accuracy保持不变的原因：样本不平衡导致的，样本分类向某一类过分倾斜了。
...
3.尝试其他超参数，例如学习率，优化器(sgd,adam。。)等



### 验证集loss震荡

[loss问题总结](https://zhuanlan.zhihu.com/p/420053831)



![](D:\github\MyKnowledgeRepository\img\machine_learning_img\deep_learning\实验结果\loss和accuracy.png)



验证集loss震荡：现象是正常的，遇到陌生的数据损失值肯定会高，loss肯定是在震荡的但整体呈下降趋势的。
...
当数据集太大时，内存撑不住。batchsize增大到一定的程度，其确定的下降方向已经基本不再变化。
...
6.数据和标签：数据分类标注是否准确？数据是否干净？数据库太小一般不会带来不收敛的问题，只要你一直在train总会收敛（跑飞了不算）。反而不收敛一般是由于样本的信息量太大导致网络不足以fit住整个样本空间。样本少只可能带来过拟合的问题。



7.网络设定不合理**：**如果做很复杂的分类任务，却只用了很浅的网络，可能会导致训练难以收敛，换网络换网络换网络，重要的事情说三遍，或者也可以尝试加深当前网络。
...
NER（Named Entity Recognition），中文称为命名实体识别，是NLP中一项非常基础的任务。命名实体一般指的是文本中具有特定意义或者指代性强的实体，通常包括**人名**、**地名**、**机构名**、**日期时间**、**专有名词**等。而命名实体识别，就是要在文本中将这些实体标注出来。

![](https://pic1.zhimg.com/80/v2-3e835663a88669b650345aa517639f40_720w.webp)



## 相关资料

[用BERT做NER](https://zhuanlan.zhihu.com/p/346828049)
