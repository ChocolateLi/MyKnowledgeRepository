...
线程中断即线程运行过程中被其他线程给打断了，它与 stop 最大的区别是：stop 是由系统强制终止线程，而线程中断则是给目标线程发送一个中断信号，目标线程接收到中断信号后，是结束线程还是执行其他取决于目标线程。
...
线程会尝试获取锁对象，其内部包含一个计数器，当计数器为0时则可以成功获取，获取后将计数器+1，执行完相应的代码后，将计数器设为0，表明锁被释放；如果获取锁失败，当前线程就会阻塞等待，直到锁被另一个线程释放为止。
...
它使用了volatile修饰的int类型变量state，来表示同步状态，通过CAS修改同步状态的值。当线程调用lock方法时，如果state=0，说明没有任何线程占有共享资源的锁，可以获得锁并将state+1；如果state不为0，则说明线程正在使用共享变量，其他线程必须加入同步队列进行等待。
